#!/bin/bash
# make sign ca_name=brbca-mtls csr_name=bxblue
# 02-sign-csr.sh -n nome_ca -c nome_csr -v validation_type

# --- Variáveis Globais ---
SRC_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PARENT_DIR="$(dirname "$SRC_DIR")"

# --- Funções de Utilitário ---
UTILS="$SRC_DIR/lib/00-util.sh"
if [[ -f "$UTILS" ]]; then
  source "$UTILS"
else
  echo "Erro: Arquivo de utilidades não encontrado em $UTILS." >&2
  exit 1
fi

newcert() {
  local ca_name="$1"
  local csr_name="$2"
  
  # Definição de caminhos para facilitar a manutenção
  local base_path="$PARENT_DIR/ca/$ca_name"
  local ca_conf_file="$PARENT_DIR/ca/$ca_name/conf/$ca_name.cnf"
  local csr_file="$base_path/csr/$csr_name/$csr_name.csr.pem"
	local cert_out_dir="$base_path/certs/$csr_name"
  local cert_file="$base_path/certs/$csr_name/$csr_name.cert.pem"
  local ca_cert="$base_path/certs/$ca_name.cert.pem"
	
  # 1. Validação da CA
  if [[ ! -d "$base_path" ]]; then
    echo "[-] Erro: CA '$ca_name' não encontrada em $base_path"
    return 1
  fi

  # 2. Busca pelo CSR mais recente (.csr.pem)
  # Procura no diretório específico e pega o arquivo com data de modificação mais recente
  if [[ -d "$csr_dir" ]]; then
    local csr_file=$(ls -t "$csr_dir"/*.csr.pem 2>/dev/null | head -n 1)
  fi

  if [[ -z "$csr_file" || ! -f "$csr_file" ]]; then
    echo "[-] Erro: Nenhum arquivo CSR encontrado para '$csr_name' em $csr_dir"
    return 1
  fi

  # 3. Preparação do diretório de saída
  # Verifica se não existe ou se não é um diretório antes de tentar criar
  if [[ ! -d "$cert_out_dir" ]]; then
    echo "[+] Criando diretório de saída: $cert_out_dir"
    mkdir -p "$cert_out_dir"
  fi

  # 4. Assinatura do Certificado
  echo "[+] Usando CSR mais recente: $(basename "$csr_file")"
  echo "[+] Assinando com a CA '$ca_name'..."
  
  openssl ca -days 365 \
    -in "$csr_file" \
    -out "$cert_file" \
	  -config "$ca_conf_file" \
		-policy policy_ev

  # 5. Verificação de Sucesso
  if [[ $? -eq 0 ]]; then
    echo "[V] Sucesso! Certificado gerado em: $cert_file"
  else
    echo "[X] Erro na assinatura do certificado."
    return 1
  fi
}

# Usa getopts para analisar as opções da linha de comando
while getopts "n:c:" opt; do
  case $opt in
    n)
      CA_NAME="$OPTARG"
      ;;
    c)
      CSR_FILE="$OPTARG"
      ;;
    \?)
      echo "Opção inválida: -$OPTARG" >&2
      exit 1
      ;;
    :)
      echo "A opção -$OPTARG requer um argumento." >&2
      exit 1
      ;;
  esac
done

# Chama a função principal com o nome do módulo obtido
newcert "$CA_NAME" "$CSR_FILE"
